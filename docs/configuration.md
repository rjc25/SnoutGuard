...
SnoutGuard is configured via `.snoutguard.yml` in your project root. Run `snoutguard init` to generate one with sensible defaults.
...
Connect to a remote SnoutGuard server instance. Omit this section entirely for local-only CLI usage.
...
```yaml
server:
  url: https://snoutguard.example.com
  api_key_env: SNOUTGUARD_API_KEY
```
...
LLM provider and model settings. SnoutGuard uses **tiered models** — different models for different operations, each independently configurable.
...
Use `snoutguard costs` to see cost estimates based on your configuration.
...
This is the default config generated by `snoutguard init`:
