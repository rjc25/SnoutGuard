...
import type { ArchDecision, SnoutGuardConfig, SyncFormat } from '@snoutguard/core';
import { createLlmClient, analyzeWithLlm } from '@snoutguard/core';
...
Start with "# Architecture" header, followed by a line: "> Generated by [SnoutGuard](https://github.com/rjc25/SnoutGuard)"
...
Start with a comment line: "# Generated by SnoutGuard (https://github.com/rjc25/SnoutGuard)"
...
- Before writing code, call get_architectural_guidance with a task description if the SnoutGuard MCP server is configured.
- After changes, run "snoutguard review --diff <branch>" to check for violations before committing.
- After significant changes, run "snoutguard analyze" then "snoutguard sync" to refresh this file.`,

  copilot: `Output format: Markdown (.github/copilot-instructions.md for GitHub Copilot).
Start with: "<!-- Generated by SnoutGuard (https://github.com/rjc25/SnoutGuard) -->" then a brief project overview paragraph.
Use ## sections for major architectural areas.
Write constraints as clear directives.

Include a "## Workflow" section at the end with rules for using SnoutGuard:
- Before coding, call get_architectural_guidance if the MCP server is configured.
- After changes, run "snoutguard review --diff <branch>" before committing.
- After significant changes, run "snoutguard analyze" then "snoutguard sync" to refresh this file.`,
...
- Before coding, call get_architectural_guidance if the SnoutGuard MCP server is configured.
- After changes, run "snoutguard review --diff <branch>" before committing.
- After significant changes, run "snoutguard analyze" then "snoutguard sync" to refresh this file.`,

  kiro: `Output format: Markdown (.kiro/steering.md for AWS Kiro).
Start with: "<!-- Generated by SnoutGuard (https://github.com/rjc25/SnoutGuard) -->"
Use ## sections for architectural areas.
Write constraints as steering directives that guide code generation.

Include a "## Workflow" section with rules for using SnoutGuard:
- Before coding, call get_architectural_guidance if the MCP server is configured.
- After changes, run "snoutguard review --diff <branch>" before committing.
- After significant changes, run "snoutguard analyze" then "snoutguard sync" to refresh this file.`,
