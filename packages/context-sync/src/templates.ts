...
import type { ArchDecision, ArchCategory } from '@snoutguard/core';
...
const USER_SECTION_START = '<!-- snoutguard:user-start -->';
const USER_SECTION_END = '<!-- snoutguard:user-end -->';
...
export function generationHeader(format: string): string {
  ...
  return `# Generated by SnoutGuard (${format}) on ${timestamp}`;
}
